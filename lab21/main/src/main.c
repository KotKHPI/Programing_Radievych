 ...
 * @mainpage
 * # Загальне завдання
 * 4. **Для** попередньо розробленого функціоналу по роботі з прикладною областю за допомогою макровизначень зробіти наступну "кастомізацію": якщо визначене макровизначенння DEBUG, то при виклику кожної функції роботи з прикладноюю областю та для модульних тестів;
 *
 * @author Radievych V.
 * @date 15-apr-2021
 * @version 1.0
...
int main() {
    ...
    printf("Загальний час роботи програми: %d sec\n", (end-begin)/1000);
#endif
    return 0;
}
